...
import { Search, Filter, ArrowUpRight, ArrowDownRight, ArrowRightLeft, FileText, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const FinancialTransactionsTable = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['admin_finance_transactions', page, size, typeFilter, statusFilter],
    queryFn: () => financialService.getTransactions(page, size, typeFilter, statusFilter),
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      ...
      case 'CREDIT':
      case 'COD_COLLECTED':
        return <div className="p-2 bg-emerald-500/10 rounded-full"><ArrowDownRight className="w-4 h-4 text-emerald-500" /></div>;
      case 'WITHDRAWAL':
      case 'PAYOUT':
      case 'DEDUCTION':
        return <div className="p-2 bg-rose-500/10 rounded-full"><ArrowUpRight className="w-4 h-4 text-rose-500" /></div>;
      default:
        return <div className="p-2 bg-indigo-500/10 rounded-full"><ArrowRightLeft className="w-4 h-4 text-indigo-500" /></div>;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'COMPLETED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30">Completed</span>;
      case 'PENDING':
      case 'PROCESSING':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30">Pending</span>;
      case 'FAILED':
      case 'REJECTED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-800 dark:bg-rose-500/20 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30">Failed</span>;
      default:
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-500/20 dark:text-gray-300 border border-gray-200 dark:border-gray-500/30">{status}</span>;
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="p-8 bg-white/70 dark:bg-gray-800/60 backdrop-blur-2xl rounded-3xl shadow-sm border border-gray-200/50 dark:border-gray-700/50 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-gray-500 dark:text-gray-400">Loading ledger data...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-8 bg-white/70 dark:bg-gray-800/60 backdrop-blur-2xl rounded-3xl shadow-sm border border-rose-200 dark:border-rose-900/50 flex items-center justify-center min-h-[400px]">
        <p className="text-rose-500 font-medium">Failed to load transactions. Please check network connection.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-2xl rounded-3xl shadow-sm border border-gray-200/50 dark:border-gray-700/50 overflow-hidden flex flex-col">
      ...
      <div className="p-6 border-b border-gray-200/50 dark:border-gray-700/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Global Ledger</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Immutable record of all financial movements</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-200/50 dark:divide-gray-700/50">
            {transactions.map((tx: any) => (
              <tr key={tx.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-750/50 transition-colors group cursor-pointer">
                <td className="px-6 py-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate max-w-[250px]">
                ...
                <td className="px-6 py-4">
                  <span className={`text-sm font-bold ${
                    ['DEPOSIT', 'CREDIT', 'COD_COLLECTED'].includes(tx.type) 
                      ? 'text-emerald-600 dark:text-emerald-400' 
                      : ['WITHDRAWAL', 'PAYOUT', 'DEDUCTION'].includes(tx.type)
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-0.5">
                    {tx.walletId ? tx.walletId.substring(0, 8) + '...' : 'System'}
                  </div>
                </td>
