...
import { 
  Package, Truck, Users, RefreshCw, CheckCircle2, 
  Wallet, Banknote, ShieldCheck, ArrowRight,
  Search, Filter, Box, AlertTriangle, Clock,
  Check, X, User
} from 'lucide-react';
...
const AgencyAdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-8 space-y-12 font-sans selection:bg-primary/30">
      {/* Header HUD */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-muted-foreground mt-4 font-bold uppercase text-[10px] tracking-[0.2em] flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          <Card className="border-none bg-accent/20 backdrop-blur-3xl rounded-[2.5rem] p-10 border border-border/40 shadow-2xl relative overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
              <div>
                <h3 className="text-2xl font-black uppercase tracking-tight">Active Orders</h3>
                <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mt-2">Operational queue ({filteredOrders.length})</p>
              </div>
                ...
                {loading ? (
                  ...
                  filteredOrders.map((order, idx) => (
                    <motion.div 
                      ...
                      className="p-6 rounded-3xl bg-accent/10 border border-border/40 hover:bg-accent/40 transition-all group relative overflow-hidden"
                    >
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] font-black text-muted-foreground/70 uppercase tracking-widest mt-1">
                  ...
                  <div className="py-20 text-center bg-accent/10 rounded-[2rem] border-2 border-dashed border-border/40">
                    ...
                    <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.3em]">No orders in sector queue</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
          ...
          <Card className="border-none bg-accent/20 backdrop-blur-3xl rounded-[2.5rem] p-10 border border-border/40 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between mb-10">
              <div>
                ...
                <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mt-2">Driver performance HUB</p>
              </div>
              ...
              {loading ? (
                ...
                drivers.map((driver, idx) => (
                  <motion.div 
                    ...
                    transition={{ delay: idx * 0.05 }}
                    className="p-5 rounded-3xl bg-accent/10 border border-border/40 hover:bg-accent/40 transition-all flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-accent/30 flex items-center justify-center text-muted-foreground">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">{driver.status}</p>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-black leading-none">{driver.todayDeliveries || 0}</p>
                      <p className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/60 mt-1">Today</p>
                    </div>
                  </motion.div>
                ))
              ) : (
                <div className="py-12 text-center bg-accent/10 rounded-[2rem] border-2 border-dashed border-border/40">
                  ...
                  <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.3em]">No drivers deployed</p>
                </div>
              )}
            </div>
          </Card>
      ...
      <AlertDialog open={!!confirmingOrderId} onOpenChange={() => setConfirmingOrderId(null)}>
        <AlertDialogContent className="bg-card border border-border/60 rounded-[2rem] p-10">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-black text-foreground uppercase tracking-tight">Confirm Cash Payment</AlertDialogTitle>
            <AlertDialogDescription className="text-muted-foreground font-bold mt-4 uppercase text-xs tracking-widest leading-relaxed">
          ...
          <AlertDialogFooter className="mt-8 gap-4">
            <AlertDialogCancel className="rounded-xl border-border/40 bg-accent/30 text-muted-foreground font-black text-[10px] uppercase tracking-widest h-12 px-8 hover:bg-accent/40">
              Abort Action
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleConfirmPayment}
              className="rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-black text-[10px] uppercase tracking-widest h-12 px-8 shadow-xl shadow-primary/20"
            >
              Confirm Reception
            </AlertDialogAction>
          </AlertDialogFooter>
